...
run_service_server() {
    ...
    docker exec -it $CONTAINER_NAME bash -c "source /opt/ros/humble/setup.bash && source /root/ros2_ws/install/setup.bash && ros2 run service_pkg service_server"
}

run_service_client() {
    echo "启动客户端节点 (Python版本)..."
    local args="${@:2}"
    docker exec -it $CONTAINER_NAME bash -c "source /opt/ros/humble/setup.bash && source /root/ros2_ws/install/setup.bash && ros2 run service_pkg service_client $args"
}
...
case "$1" in
    ...
    *)
        ...
        echo "  service_client - 启动客户端节点 (可带两个数字参数，如: ./ros2_ws.sh service_client 3 5)"
